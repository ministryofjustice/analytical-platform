module "s3_bucket_source_zip_test" {
  ...
  bucket                  = local.splink_source_zip_bucket_test_name
  force_destroy           = false
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_source_zip_bucket_test_name}", "arn:aws:s3:::${local.splink_source_zip_bucket_test_name}/*"]
      ...
      },
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_source_zip_bucket_test_name}"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = [local.splink_s3_source_zip_write_test_bucket_key_user_arns] } }
      },
      ], [
      {
      ...
      },
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_source_zip_bucket_test_name}"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = concat(local.splink_s3_source_zip_read_test_bucket_key_user_arns, [local.splink_s3_source_zip_write_test_bucket_key_user_arns]) } }
      },
      {
  ...
  logging = {
    ...
    target_prefix = "s3access/${local.splink_source_zip_bucket_test_name}/"
  }
  lifecycle_rule = [{
...
resource "aws_cloudwatch_event_rule" "s3_bucket_source_zip_event_rule_test" {
  name        = "splink-source-zip-bucket-event-rule-test"
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_source_zip_test.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_s3_bucket_notification" "source_zip_bucket_notification_test" {
  bucket      = module.s3_bucket_source_zip_test.s3_bucket_id
  eventbridge = true
}

resource "aws_cloudwatch_event_target" "source_zip_bucket_event_target_test" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_source_zip_event_rule_test.name
