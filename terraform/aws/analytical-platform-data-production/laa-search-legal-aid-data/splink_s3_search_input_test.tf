module "s3_bucket_search_input_test" {
  ...
  bucket        = local.splink_search_input_bucket_test_name
  force_destroy = false
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_search_input_bucket_test_name}", "arn:aws:s3:::${local.splink_search_input_bucket_test_name}/*"]
      ...
      },
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_search_input_bucket_test_name}"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = concat(local.splink_s3_search_input_read_test_bucket_key_user_arns, [local.splink_s3_search_input_write_test_bucket_key_user_arns]) } }
      },
      {
  ...
  logging = {
    ...
    target_prefix = "s3access/${local.splink_search_input_bucket_test_name}/"
  }
  ...
  tags = merge(local.tags, { Name = local.splink_search_input_bucket_test_name })
}
...
resource "aws_cloudwatch_event_rule" "s3_bucket_search_input_event_rule_test" {
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_search_input_test.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_s3_bucket_notification" "search_input_bucket_notification_test" {
  bucket      = module.s3_bucket_search_input_test.s3_bucket_id
  eventbridge = true
}

resource "aws_cloudwatch_event_target" "search_input_bucket_event_target_test" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_search_input_event_rule_test.name
