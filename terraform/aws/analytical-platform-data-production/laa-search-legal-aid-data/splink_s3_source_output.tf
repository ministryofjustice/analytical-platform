...
module "s3_bucket_source_output" {
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_source_output_bucket_name}", "arn:aws:s3:::${local.splink_source_output_bucket_name}/*"]
      ...
      ], [
      {
        ...
        Action    = ["s3:PutObject"]
        Resource  = "arn:aws:s3:::${local.splink_source_output_bucket_name}/*"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = local.splink_s3_source_output_write_bucket_key_user_arns } }
      },
      {
      ...
      },
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_source_output_bucket_name}/*"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = local.splink_s3_source_output_read_bucket_key_user_arns } }
      },
      {
  ...
  tags = merge(local.tags, { Name = local.splink_source_output_bucket_name })
}

# The following resources S3 Bucket ownership control and notification
resource "aws_s3_bucket_ownership_controls" "source_output_prod" {
  bucket = module.s3_bucket_source_output.s3_bucket_id
...
resource "aws_s3_bucket_notification" "source_output_bucket_notification_prod" {
  bucket      = module.s3_bucket_source_output.s3_bucket_id
  eventbridge = true
}

# The following resources are Cloudwatch event rule and target
resource "aws_cloudwatch_event_rule" "s3_bucket_source_output_event_rule_prod" {
  name        = "splink-source-output-bucket-event-rule-prod"
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_source_output.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_cloudwatch_event_target" "source_output_bucket_event_target_prod" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_source_output_event_rule_prod.name
  target_id = "s3-event-target-prod-source-output"
