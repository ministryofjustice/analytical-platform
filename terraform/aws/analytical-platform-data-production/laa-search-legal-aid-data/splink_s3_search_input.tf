# The following bucket is laa-splink-search-input.
# This bucket shouldn't have object Lock
module "s3_bucket_search_input" {
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_search_input_bucket_name}", "arn:aws:s3:::${local.splink_search_input_bucket_name}/*"]
      ...
      },
      {
        ...
        Action    = ["s3:PutObject"]
        Resource  = "arn:aws:s3:::${local.splink_search_input_bucket_name}/*"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = local.splink_s3_search_input_write_bucket_key_user_arns } }
      },
      {
        ...
        Condition = { ArnNotEquals = {
          "aws:PrincipalArn" = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/airflow-production-laa-search-index-search"
        } }
      },
      {
  ...
  tags = merge(local.tags, { Name = local.splink_search_input_bucket_name })
}

# The following resources S3 Bucket ownership control and notification
resource "aws_s3_bucket_ownership_controls" "search_input_prod" {
  bucket = module.s3_bucket_search_input.s3_bucket_id
...
resource "aws_s3_bucket_notification" "search_input_prod_bucket_notification" {
  bucket      = module.s3_bucket_search_input.s3_bucket_id
  eventbridge = true
}

# The following resources are Cloudwatch event rule and target
resource "aws_cloudwatch_event_rule" "s3_bucket_search_input_event_prod" {
  name        = "splink-search-input-bucket-event-rule-prod"
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_search_input.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_cloudwatch_event_target" "search_input_bucket_event_target_prod" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_search_input_event_prod.name
  target_id = "s3-event-target-prod-search-input"
