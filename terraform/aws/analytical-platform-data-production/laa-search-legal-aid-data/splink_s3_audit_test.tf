#The following is the Audit Test environment Bucket
module "s3_bucket_audit_test" {
  ...
  bucket                  = local.splink_audit_bucket_test_name
  force_destroy           = false
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_audit_bucket_test_name}", "arn:aws:s3:::${local.splink_audit_bucket_test_name}/*"]
