  ...
  policy = jsonencode({
    ...
    Statement = concat([
      ...
      },
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_source_bucket_test_name}"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = concat(local.splink_s3_source_read_test_bucket_key_user_arns, [local.splink_s3_source_write_test_bucket_key_user_arns]) } }
      }
    ])
  })
...
resource "aws_s3_bucket_ownership_controls" "source_test" {
  bucket = module.s3_bucket_source_test.s3_bucket_id
...
resource "aws_cloudwatch_event_rule" "s3_bucket_source_event_rule_test" {
  name        = "splink-source-bucket-event-rule-test"
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_source_test.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_s3_bucket_notification" "source_bucket_notification_test" {
  bucket      = module.s3_bucket_source_test.s3_bucket_id
  eventbridge = true
}

resource "aws_cloudwatch_event_target" "source_bucket_event_target_test" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_source_event_rule_test.name
