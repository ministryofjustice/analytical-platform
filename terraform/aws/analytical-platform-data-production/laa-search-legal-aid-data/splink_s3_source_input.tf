# The following bucket is laa-splink-source-file-input.
# This bucket shouldn't have object Lock
module "s3_bucket_source_input" {
    ...
    Statement = concat([
      {
        ...
        Principal = "*"
        Action    = "s3:*"
        Resource  = ["arn:aws:s3:::${local.splink_source_input_bucket_name}", "arn:aws:s3:::${local.splink_source_input_bucket_name}/*"]
      ...
      ], [
      {
        ...
        Resource  = "arn:aws:s3:::${local.splink_source_input_bucket_name}/*"
        Condition = { ArnNotEquals = { "aws:PrincipalArn" = local.splink_s3_source_input_read_bucket_key_user_arns } }
      },
      {
...
resource "aws_s3_bucket_ownership_controls" "source_input_prod" {
  bucket = module.s3_bucket_source_input.s3_bucket_id
...
resource "aws_s3_bucket_notification" "source_input_bucket_notification_prod" {
  bucket      = module.s3_bucket_source_input.s3_bucket_id
  eventbridge = true
}

# The following resources are Cloudwatch event rule and target
resource "aws_cloudwatch_event_rule" "s3_bucket_source_input_event_rule_prod" {
  name        = "splink-source-input-bucket-event-rule-prod"
  ...
  event_pattern = jsonencode({
    ...
    detail = {
      bucket = {
        name = [
          module.s3_bucket_source_input.s3_bucket_id
        ]
      }
    }
  })
...
resource "aws_cloudwatch_event_target" "source_input_bucket_event_target_prod" {
  rule      = aws_cloudwatch_event_rule.s3_bucket_source_input_event_rule_prod.name
  target_id = "s3-event-target-prod-source-input"
