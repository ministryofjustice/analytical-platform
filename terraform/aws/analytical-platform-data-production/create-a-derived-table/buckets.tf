    ...
    rules = [
      {
        ...
        destination = {
          ...
          encryption_configuration = {
            replica_kms_key_id = "arn:aws:kms:${local.destination_region}:${var.account_ids["analytical-platform-compute-production"]}:key/${data.aws_kms_alias.mojap_apc_prod_cadet_replication_bucket.target_key_id}"
          }
          metrics = {
            status  = "Enabled"
            minutes = 15
