    ...
    content {
      ...
      resources = flatten([
        for pattern in statement.value.database_string_pattern : [
          "arn:aws:glue:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:database/${pattern}",
          "arn:aws:glue:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:table/${pattern}/*"
        ]
      ])
      principals {
        type        = "AWS"
