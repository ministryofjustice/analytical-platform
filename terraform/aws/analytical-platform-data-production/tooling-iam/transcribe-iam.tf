data "aws_iam_policy_document" "transcribe_service_access" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:transcribe:${data.aws_region.current.region}:${var.account_ids["analytical-platform-data-production"]}:vocabulary/*",
      "arn:aws:transcribe:${data.aws_region.current.region}:${var.account_ids["analytical-platform-data-production"]}:transcription-job/*",
      "arn:aws:transcribe:${data.aws_region.current.region}:${var.account_ids["analytical-platform-data-production"]}:language-model/*"
    ]
  }
  statement {
