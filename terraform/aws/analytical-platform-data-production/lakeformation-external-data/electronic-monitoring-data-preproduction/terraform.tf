terraform {
  backend "s3" {
    ...
    encrypt        = true
    key            = "aws/analytical-platform-data-production/lakeformation-external-data/electronic-monitoring-data-preproduction/terraform.tfstate"
    region         = "eu-west-2"
...
provider "aws" {
  ...
  region = "eu-west-2"
  assume_role {
    role_arn = "arn:aws:iam::${local.account_ids["electronic-monitoring-data-preproduction"]}:role/analytical-platform-data-production-share-role"
  }
  default_tags {
    tags = var.tags
  }
