terraform {
  backend "s3" {
    ...
    encrypt        = true
    key            = "aws/analytical-platform-data-production/lakeformation-external-data/digital-prisons-reporting-development/terraform.tfstate"
    region         = "eu-west-2"
...
provider "aws" {
  alias  = "digital_prisons_reporting_dev_eu_west_2"
  region = "eu-west-2"
  assume_role {
    role_arn = "arn:aws:iam::${local.account_ids["digital-prisons-reporting-development"]}:role/analytical-platform-data-production-share-role"
  }
  default_tags {
    tags = var.tags
  }
