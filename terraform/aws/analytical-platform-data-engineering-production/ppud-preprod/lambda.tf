data "aws_iam_policy_document" "copy_object" {
  statement {
    ...
    resources = [
      module.ppud_preprod.s3_bucket_arn,
      "${module.ppud_preprod.s3_bucket_arn}/*"
    ]
  }
...
module "copy_object" {
  ...
  environment_variables = {
    LAND_BUCKET           = module.ppud_preprod.s3_bucket_id
...
resource "aws_cloudwatch_event_rule" "check_recent_file_daily" {
  ...
  description         = "Invoke recent-file checker daily at 15:50 UTC"
  schedule_expression = "cron(15 50 * * ? *)"
}
