resource "aws_iam_role" "preprod_bastion_ec2" {
  name = "preprod-bastion-ec2"
...
resource "aws_iam_role_policy_attachment" "preprod_bastion_ec2_ssm" {
  role       = aws_iam_role.preprod_bastion_ec2.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# Instance Profile for EC2
resource "aws_iam_instance_profile" "preprod_bastion_ec2" {
  name = "preprod-bastion-ec2-instance-profile"
  role = aws_iam_role.preprod_bastion_ec2.name
}
...
resource "aws_instance" "preprod_bastion_ec2" {
  ...
  iam_instance_profile   = aws_iam_instance_profile.preprod_bastion_ec2.name
  subnet_id              = module.vpc.private_subnets[0]
  vpc_security_group_ids = [aws_security_group.preprod_bastion_ec2.id]
  root_block_device {
