...
from config import  MODEL_ID, REGION, QUERY_ANALYSER_SYSTEM_PROMPT,GUARDRAIL_ID,GUARDRAIL_VERSION
...
"""
Query_analyser(llm, QUERY_ANALYSER_SYSTEM_PROMPT)

The `QueryAnalyser` decides intent and other factors by:

1. **Sending the query to Claude LLM** via AWS Bedrock with your detailed `QUERY_ANALYSER_SYSTEM_PROMPT`

2. **The LLM analyzes** the query based on instructions in the system prompt:
   - Maps keywords to tools (e.g., "Airflow" → Airflow pages)
   - Classifies intent type (how-to, troubleshooting, policy, etc.)
   - Assesses complexity (low/medium/high) based on ambiguity and specificity
   - Determines confidence score
   - Recommends retrieval strategy (filtered/hybrid/broad)
   - Suggests metadata filters (`page_h1`, `root_heading`)
   - Extracts entities (tools, sections, topics)

3. **Returns structured JSON** following the response format defined in your prompt

4. **The code parses** this JSON into a `QueryAnalysis` dataclass with safe defaults

**In short**: The LLM does the semantic analysis based on your prompt guidelines; the Python code just orchestrates the API call and structures the output.
...
2. Sent to Claude LLM with System Prompt
Query: "How to setup RStudio?"
System Prompt: QUERY_ANALYSER_SYSTEM_PROMPT (your detailed instructions)

3. LLM Analysis (what Claude returns as JSON):
{
  "intent": {
    "primary": "learn_rstudio_setup_process",
    "secondary": [],
    "type": "how-to",
    "user_need": "User wants step-by-step instructions to set up RStudio"
  },
  "complexity": {
    "level": "low",
    "reasons": ["single_tool_focus", "clear_action_setup", "straightforward_question"],
  ...
  "retrieval_strategy": {
    ...
    "reasoning": "Clear single tool with specific action (setup)",
    "suggested_top_k": 5,
  ...
  "next_step_guidance": "Apply BOTH filters: page_h1 (RStudio) + root_heading (Getting started sections)"
}


The LLM analysis is entirely based on the prompt (QUERY_ANALYSER_SYSTEM_PROMPT).

The prompt contains:

Instructions on how to classify queries
Metadata schema (page_h1, root_heading structure)
Tool mappings (keywords → page titles)
Section mappings (actions → root_heading values)
Complexity guidelines (low/medium/high rules)
Strategy rules (when to use filtered/hybrid/broad)
Response format (exact JSON structure to return)
Examples (showing expected analysis for different query types)
