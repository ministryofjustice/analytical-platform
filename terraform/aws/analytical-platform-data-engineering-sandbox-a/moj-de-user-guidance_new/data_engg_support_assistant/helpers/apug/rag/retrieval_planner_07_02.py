...
""" 
Retrieval Planner - it converts the QueryAnalyzer's output into an actionable retrieval plan.


The planner validates, adjusts, and safeguards the analyzer's suggestions, ensuring retrieval doesn't fail due to invalid/over-strict filters.

Python logic layer that sits before KB retrieval
Does: Decides HOW to query the KB (filters, top_k, fallback strategy)
Uses kb_catalog (optional) to validate filters, but doesn't access the actual KB
