...
resource "aws_lakeformation_permissions" "table_grants_describe_only" {
  for_each = { for s in local.user_grants_with_row_filter : "${s.role_name}-${s.database_name}-${s.table_name}" => s }
  provider = aws.source
...
resource "aws_lakeformation_permissions" "table_grants_describe_select" {
  for_each = {
    for s in local.user_grants_all_rows :
    "${s.role_name}-${s.database_name}-${s.table_name}" => s
  }
  ...
  table {
    name          = each.value.table_name != "*" ? each.value.table_name : null
    database_name = each.value.database_name
    wildcard      = each.value.table_name == "*" ? true : null
  }
}
