      ...
      combo_key => {
        ...
        severity => join("\n", compact(flatten([
          ...
          [
            ...
            "                    params:",
            "                      - ${local.thresholds[env][severity == "warning" ? combo.rule.warn_key : combo.rule.crit_key]}",
          ],
          ...
          contains(["baseline_gt", "baseline_lt"], combo.rule.type) ? [
            ...
            "              type: math",
            "              refId: D",
            "              expression: ${combo.rule.type == "baseline_lt" ? "($B - $BASE_R) / $BASE_R * 100 < -${local.thresholds[env][severity == "warning" ? combo.rule.warn_key : combo.rule.crit_key]}" : "($B - $BASE_R) / $BASE_R * 100 > ${local.thresholds[env][severity == "warning" ? combo.rule.warn_key : combo.rule.crit_key]}"}",
