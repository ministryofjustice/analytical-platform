...
def start_query_execution_and_wait(
        ...
        while response := athena_client.get_query_execution(
...
def handler(event,context,athena_client=athena_client):
    ...
    athena_results = athena_client.get_query_results(QueryExecutionId=response['QueryExecutionId'])

    formated_result= format_athena_results_to_table(athena_results)

    log_message = formated_result if formated_result == "No data to display" else "Results fetched successfully"
