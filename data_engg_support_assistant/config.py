...
# ============================================================================
NUM_RETRIEVAL_RESULTS = int(os.getenv("DEFAULT_NUMBER_OF_RESULTS", "5"))
...
GUARDRAIL_VERSION = os.getenv('GUARDRAIL_VERSION', None)
...
# ============================================================================
if IS_LAMBDA:
    # Lambda uses API Gateway Authorizer
    AUTH_TOKEN = None
else:
    # Local development
    AUTH_TOKEN = os.getenv('AUTH_TOKEN')
    if not AUTH_TOKEN:
        raise ValueError("AUTH_TOKEN must be set for local development")
...
if IS_LAMBDA:
    if Path("/asset").exists():
        BASE_DIR = Path("/asset")
    elif Path("/var/task").exists():
        BASE_DIR = Path("/var/task")
    else:
        raise RuntimeError("Lambda base directory not found")
else:
...
if CATALOG_PATH.exists():
    print(f"[CONFIG] Catalog found: {CATALOG_PATH}")
else:
    print(f"[CONFIG] Warning: Catalog not found at {CATALOG_PATH}")
...
if not IS_LAMBDA:
    print("="*70)
    print("  Configuration Loaded (Local Development)")
    print("="*70)
    print(f"  KB_ID:     {KB_ID[:4]}***")
    print(f"  MODEL_ID:  {MODEL_ID}")
    print(f"  REGION:    {REGION}")
    print(f"  AUTH:      {AUTH_TOKEN[:8]}***")
    print(f"  BASE_DIR:  {BASE_DIR}")
    print(f"  RESULTS:   {NUM_RETRIEVAL_RESULTS}")
    print(f"  MAX_CTX:   {MAX_CONTEXT_TOKENS} tokens")
