...
from config import MODEL_ID, REGION,GUARDRAIL_ID, GUARDRAIL_VERSION,USE_GUARDRAILS
...
Provide your answer with inline citations:"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Add guardrails if configured - FLATTENED, NOT NESTED
                if USE_GUARDRAILS and GUARDRAIL_ID and GUARDRAIL_VERSION:
                    invoke_params['guardrailIdentifier'] = GUARDRAIL_ID
                    invoke_params['guardrailVersion'] = GUARDRAIL_VERSION
                    print(f"[GUARDRAIL] Applying guardrail: {GUARDRAIL_ID} (v{GUARDRAIL_VERSION})")
                ...
                print(f"[DEBUG] Stop reason: {payload.get('stop_reason')}")
                print("=" * 70)
                ...
                print(f"[GUARDRAIL ERROR] {type(e).__name__}: {str(e)}")
                print(f"[GUARDRAIL ERROR] Full traceback:")
                traceback.print_exc()
