    ...
    for block in body.get('blocks', []):
        if block['type'] == 'section':
            ...
            if 'Request ID:' in metadata:
                ...
                match = re.search(r'`([a-f0-9-]+)`', metadata)
                request_id = match.group(1) if match else "unknown"
    """

    for block in body.get('blocks', []):
        block_type = block.get('type')
        
        # Extract answer (main section block)
        if block_type == 'section':
            text_obj = block.get('text', {})
            text = text_obj.get('text', '')
            
            # Differentiate answer vs sources
            if text and not text.startswith('*📚 Sources:*'):
                answer = text
            elif '* Sources:*' in text:
                sources_text = text
        
        # Extract metadata from context blocks
        elif block_type == 'context':
            elements = block.get('elements', [])
            if elements:
                metadata = elements[0].get('text', '')
                
                # Extract request_id (handles both old and new formats)
                if 'Request ID:' in metadata:
                    try:
                        # New format: *Request ID:* `abc123`
                        if '*Request ID:*' in metadata:
                            request_id = metadata.split('*Request ID:* `')[1].split('`')[0]
                        # Old format: Request ID: `abc123`
                        else:
                            request_id = metadata.split('Request ID: `')[1].split('`')[0]
                ...
                # Extract confidence
                if 'Confidence:' in metadata or '*Confidence:*' in metadata:
                    confidence_text = metadata
